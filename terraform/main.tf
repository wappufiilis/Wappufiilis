...
resource "aws_iam_policy" "lambda_dynamodb" {
  ...
  path        = "/"
  description = "IAM policy for putting items to DynamoDB from a lambda"
...
resource "aws_iam_role_policy_attachment" "lambda_reminder_dynamodb" {
  role       = aws_iam_role.iam_for_lambda.name
  policy_arn = aws_iam_policy.lambda_dynamodb_reminder.arn
}

# This is to optionally manage the CloudWatch Log Group for the Lambda Function.
# If skipping this resource configuration, also add "logs:CreateLogGroup" to the IAM policy below.
resource "aws_cloudwatch_log_group" "example" {
  name              = "/aws/lambda/wappufiilis_bot_lambda_${var.environment}"
